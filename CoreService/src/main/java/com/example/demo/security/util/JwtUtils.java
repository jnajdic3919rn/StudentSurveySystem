...
public class JwtUtils {
    ...
    public String generateToken(UserDetails userDetails, String faculty) {
        ...
        return generateToken(claims, ((SecurityUser) userDetails).getUsername());
    }

    public String generateTokenGoogle(UserResponse userResponse) {
        Map<String, Object> claims = new HashMap<>();
        claims.put("roles", List.of(userResponse.getRole()));
        claims.put("faculty", userResponse.getFaculty());
        return generateToken(claims, userResponse.getUsername());
    }
    ...
    public List<GrantedAuthority> getAuthoritiesFromToken(String jwtToken) throws CantParseJwtException {
        List<String> permissions = getClaimFromToken(jwtToken, claims -> claims.get("roles", List.class));
